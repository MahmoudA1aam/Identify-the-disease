  ...
  void getImageLabels(XFile image) async {
    ...
    for (int i = 0; i <= labels.length; i++) {
      ...
      if ((labels[i].label == "Skin" && confidence >= 80) ||
        ...
        setState(() {});
        SnackBarService.showSuccessMessage("sucess");

        return;
      } else if (labels[i].label == "Skin" && confidence > 80) {
        successImage = false;
        SnackBarService.showErrorMessage("Take other photo more details");
        setState(() {});
      } else {
        successImage = false;
        SnackBarService.showErrorMessage("Take other photo");
        setState(() {});
      }
                  ...
                  child: Center(
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          border: Border.all(
                              color:
                                  successImage ? Colors.green : Colors.grey,
                              width: 3)),
                    ),
                  ),
                ),
